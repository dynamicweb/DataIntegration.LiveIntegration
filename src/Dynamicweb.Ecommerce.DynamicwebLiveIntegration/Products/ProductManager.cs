...
namespace Dynamicweb.Ecommerce.DynamicwebLiveIntegration.Products
{
    ...
    public static class ProductManager
    {
        ...
        [Obsolete("Settings.GetProductCacheLevel() instead")]
        public static Integration.CacheLevel GetProductCacheLevel(Settings settings)
        {
            var level = settings.GetProductCacheLevel();
            return level == ResponseCacheLevel.Session ? Integration.CacheLevel.Session : Integration.CacheLevel.Page;
        }

        [Obsolete("Use ProductManager.GetPrices instead")]
        public static List<Price> GetProductPrices(Integration.ProductInfo productInfo)
        {
            List<Price> prices = new List<Price>();
            if (productInfo != null)
            {
                prices = ProductProvider.GetProductPrices(productInfo);
            }
            return prices;
        }

        [Obsolete("Use GetProductInfo(Product product, Settings settings, User user) instead")]
        public static Integration.ProductInfo GetProductInfo(Settings settings, Product product)
        {
            return GetProductInfo(settings, product, User.GetCurrentExtranetUser());
        }

        [Obsolete("Use GetProductInfo(Product product, Settings settings, User user) instead")]
        public static Integration.ProductInfo GetProductInfo(Settings settings, Product product, User user)
        {
            return ConvertToProductInfo(GetProductInfo(product, settings, user));
        }

        [Obsolete("Don't use this")]
        internal static Integration.ProductInfo ConvertToProductInfo(ProductInfo info)
        {
            if (info == null)
                return null;

            var result = new Integration.ProductInfo();
        ...
        [Obsolete("Don't use this")]
        internal static ProductInfo ConvertToProductInfo(Integration.ProductInfo info)
        {
            if (info == null)
                return null;
